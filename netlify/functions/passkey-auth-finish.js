...
exports.handler = async function handler(event) {
  try {
    ...
    if (!credential || !credential.id || !credential.response) {
      return json(400, { ok: false, error: "Credential Face ID tak sah." });
    }

    const done = await finishAuthenticationForPhone({ phone, credential });
    ...
    return json(200, {
      ...
      message: "Login Face ID berjaya."
    });
  } catch (e) {
    console.error("passkey-auth-finish error:", e);
    return json(500, {
      ok: false,
